...
import type { DailyMetric, GBPConfig } from '../types.js';

const API_BASE = 'https://businessprofileperformance.googleapis.com/v1';

async function apiRequest(config: GBPConfig, path: string): Promise<unknown> {
...
export async function fetchMultiDailyMetrics(
    startDate: string,
    endDate: string,
    metrics?: DailyMetric[]
) {
...
export async function getSearchKeywords(startDate: string, endDate: string, pageSize = 20, pageToken?: string) {
    const config = getConfig();
    const location = `locations/${config.locationId}`;
...
export const metricsTools = {
    gbp_get_performance: {
        ...
        handler: async (args: { startDate: string; endDate: string; metrics?: DailyMetric[] }) => {
    ...
    gbp_get_search_keywords: {
        ...
        handler: async (args: { startDate: string; endDate: string; pageSize?: number; pageToken?: string }) => {
