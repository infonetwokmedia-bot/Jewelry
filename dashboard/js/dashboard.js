...
(function () {
  ...
  async function loadGoldTicker() {
    ...
    // Don't show yet — fetchAndRenderGold will show it only on success
  ...
  async function fetchAndRenderGold() {
    ...
    try {
      ...
      if (d.success !== false) {
        ...
        // API returned error — hide ticker quietly
        if (bar) bar.style.display = "none";
        console.warn("Gold ticker:", d.error || "API error");
      }
    } catch (e) {
      // Network/auth error — hide ticker quietly, don't break UI
      if (bar) bar.style.display = "none";
